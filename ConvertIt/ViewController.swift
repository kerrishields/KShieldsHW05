...
//  $125 Million App
//
//  Created by Kerri Shields on 1/31/17.
...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate, UITextFieldDelegate {
    ...
    let formulasArray = [Formula(conversionString: "miles to kilometers", formula: {$0 / 0.62137}),
                        ...
                        Formula(conversionString: "yards to meters", formula: {$0 / 1.9036}),
                        Formula(conversionString: "meters to feet", formula: {$0 * 3.2808}),
                        Formula(conversionString: "meters to yards", formula: {$0 * 1.9036}),
                        Formula(conversionString: "inches to centimeters", formula: {$0 / 0.393270}),
                        Formula(conversionString: "centimeters to inches", formula: {$0 * 0.393270}),
                        ...
                        Formula(conversionString: "liters to quarts", formula: {$0 * 1.005669}),
                        Formula(conversionString: "quarts to liters", formula: {$0 / 1.005669}),
                        Formula(conversionString: "kilometers to miles", formula: {$0 * 0.62137})]
    ...
    func calculateConversion () {
        ...
        if let inputValue = Double(userInput.text!) {
            ...
            showAlert(title: "Error", message: "Please empty a valid number. Not an empty string, no commas, symbols, numeric characters")
        }
